...
- Subespacios y proyección ortogonal — Gram-Schmidt (también paso a paso: la sombra de v₂ sobre u₁ y lo que queda), proyección ortogonal con su matriz P_W, proyección oblicua a lo largo de una dirección U, residuo y complemento ortogonal; con un segundo subespacio, W₁ ∩ W₂ (dibujada) y W₁ + W₂ con la fórmula de Grassmann.
...
**Geometría**
- Regla y compás — construcciones que se mantienen al mover los puntos: cada objeto guarda su definición, no sus coordenadas. Herramientas por grupos: puntos (libre, sobre un objeto, medio, intersección, centro), rectas (segmento, recta, semirrecta, vector, perpendicular, paralela, mediatriz, bisectriz, tangentes), polígonos (y regulares), circunferencias (centro y punto, centro y radio, por tres puntos, compás, arco, semicircunferencia), cónicas (elipse e hipérbola por focos, parábola por foco y directriz, por cinco puntos), circunferencia inscrita en un triángulo, medidas (ángulo, distancia, distancia punto–recta, área, perímetro, pendiente), vectores (módulo, producto escalar, ángulo entre vectores o rectas, proyección de un vector sobre otro y su componente perpendicular, base ortonormal por Gram–Schmidt), transformaciones (simetría axial y central, rotación, traslación, homotecia, inversión y proyección ortogonal de cualquier objeto sobre una recta) y lugar geométrico. Cada cónica dice su tipo (elipse, circunferencia, hipérbola, parábola, degenerada), su excentricidad y su centro. Con Mover se arrastran los puntos libres (azules) y los que van sobre un objeto (verdes). La lista de objetos da coordenadas, ecuaciones y medidas, y cada uno se oculta o se borra con todo lo que depende de él.
- Espacio 3D — una fila por objeto: superficies implícitas (`x^2+y^2+z^2=4`, planos), `z = f(x, y)` escrita como `x^2 - y^2`, puntos arrastrables `A = (1, 2, 3)`, curvas `(cos(t), sin(t), t/4), -12 < t < 12`, superficies paramétricas en u, v, y órdenes `recta`, `segmento`, `vector`, `plano(A, B, C)`, `esfera(C, r)`, `cubo`, `prisma`, `piramide`, `cono`, `cilindro` y los poliedros regulares (con volumen y área), `revolucion(f(x), a, b)` y `corte(1, 2)`, la curva donde se cortan dos superficies. Proyecciones y medidas: `proy(P, π)`, `proy(v, w)`, `perp(v, π)`, `simetrico(P, r)`, `dist(r, s)` (también entre rectas que se cruzan), `angulo(v, π)`, `norma(v)`, `escalar(u, v)`, `vectorial(u, v)` y `gram(u, v, w)`; los objetos se nombran (`r = recta(A, B)`) o se citan por su número de fila, y un plano escrito como ecuación también vale. Las letras libres son deslizadores.
...
- Campos vectoriales — F = (P, Q, R) escrita a mano o de la lista (radial, dipolo, hilo con corriente, torbellino…), o un campo escalar φ con su gradiente ∇φ y su laplaciano ∇²φ calculados con el CAS y la superficie de nivel que pasa por la sonda; flechas coloreadas por el módulo, líneas de campo integradas, divergencia y rotacional en la sonda, derivada direccional D_û φ y proyección de F sobre una dirección û.
...
**Física**
- Electrostática — cargas arrastrables con líneas de campo y equipotenciales, flujo de Gauss por una superficie y método de las imágenes (plano y esfera a tierra); en la sonda se comprueban E = −∇V, ∇·E = 0, ∇×E = 0 y ∇²V = 0.
- Magnetostática — Biot–Savart sobre curvas escritas (hilo, espira, Helmholtz, solenoide, nudo de trébol), líneas de B y circulación de Ampère; en P, ∇·B y ∇×B.
